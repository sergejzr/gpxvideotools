...
public class GPXRouteBuilder {
	...
	public DiscreteRoute makeRoute(List<WayPoint> track) {
		DiscreteRoute ret = new DiscreteRoute();
		...
		for(WayPoint wp:speedenhanced) 
		{
			...
			double slope = distance<1?prevRT.getSlope():(wp.getElevation().get().doubleValue()-prev.getElevation().get().doubleValue())/distance;
			...
			rp=rp.setAcceleration(speeddiff/timediff).setDistance(distance+prevRT.getDistance()).setSlope(slope).setHeading(resultDegree); 
			ret.put(rp);
	...
	private void createRoute(int track, int segment) {
		...
		latfunc = GPXInterpolatorA.interpolate(starttime, mytrack, GPXTargetValue.LAT, itype);
		lonfunc = GPXInterpolatorA.interpolate(starttime, mytrack, GPXTargetValue.LON, itype);
		elevationfunc = GPXInterpolatorA.interpolate(starttime, mytrack, GPXTargetValue.ELEVATION, itype);
	...
	public static Hashtable<GPXTargetValue, List<Double>> toDoubleArrays(List<WayPoint> in, GPXTargetValue... enhancements)
			throws Exception {
	...
	public static void main(String[] args) {
		try {
			...
			ArrayList<WayPoint> sm = rb.smooth(mt, 8,
					new GPXTargetValue[] { GPXTargetValue.LAT, GPXTargetValue.LON, GPXTargetValue.ELEVATION });
			
			List<WayPoint> fb = rb.fillGaps(sm);
			List<WayPoint> enhanced = rb.addSpeed(fb);
			rb.print(rb.addSpeed(enhanced));
*/
			/*
			 * List<WayPoint> mt = rb.fillGaps(rb.getMytrack());
			 * 
		...
		for (WayPoint wp : l) {
			...
			System.out.println(
					...
					sdf.format(Date.from(wp.getTime().get().toInstant())) + "\t" + wp.getLongitude() + "\t"
							...
							+ getVal(wp.getElevation()) + "\t" + (speed == null ? "" : GPXRouteBuilderX.kmph(speed))
