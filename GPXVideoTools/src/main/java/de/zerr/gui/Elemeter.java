 ...
 *
 */
public class Elemeter extends JPanel {
	...
	public static Elemeter fromRoute(ContinuousRoute route) {
		...
		while(!startpoint.isAfter(endpoint)) 
    	{
    		...
    		double el=rp.getElevation();
    		theway.put(startpoint,series1.getItemCount());
    		series1.add(curkm-startdist,el); 
		...
		try {
			for (double i = startKm.getDistance(); i < endKm.getDistance();i+=1) {
		...
		*/
		...
		Elemeter ret = new Elemeter(series1);
		ret.theway=theway;
		return ret;
	}

	public Elemeter(XYSeries series12) {
	...
	public void setTime(ZonedDateTime curTime) {
		...
		if  (bikeimage==null) 
		{
			...
			try {
				bikeimage=ImageIO.read(new  File("/home/szerr/git/gpxvideotools/GPXVideoTools/img/icons/bike.png"));
				//imgwidth=bikeimage.getWidth(null);
				//imgheight=bikeimage.getHeight(null);
			} catch (IOException e) {
				// TODO Auto-generated catch block
				e.printStackTrace();
			}
		...
		plot.addAnnotation(pointer2 = new XYImageAnnotation(series1.getDataItem(idx).getX().doubleValue(),
