...
public class ContinuousRoute implements Route {
	...
	private ContinuousRoute(ZonedDateTime gpxstarttime) {
	...
	public static ContinuousRoute fromDiscrete(DiscreteRoute from, InterpolationType itype, GPXTargetValue... targets)
			throws Exception {
		Hashtable<GPXTargetValue, UnivariateFunction> interpolated = new Hashtable<GPXTargetValue, UnivariateFunction>();
		ContinuousRoute ret = new ContinuousRoute(from.getGpxstarttime());
		...
		ret.bounds = from.getBounds(ret.getStartTime(), ret.getEndtime());
	...
	/**
	 * 
	 * @return Rectangle of the routhe x,y are the center coordinates (lat, lon).
	 *         w,h are the width and height in meters
	 */
	public Rectangle2D getGPXBounds() {
		return bounds;
	}
	...
	public static Hashtable<GPXTargetValue, List<Double>> toDoubleArrays(Collection<RoutePoint> in,
			GPXTargetValue... targets) throws Exception {
	...
	public Iterator<RoutePoint> iterateBy(ZonedDateTime start, ZonedDateTime stop, Duration resolution) {
		ContinuousRoute cr = this;
		return new Iterator<RoutePoint>() {

			ZonedDateTime curTime = start;
			ZonedDateTime stopTime = stop;
			...
			public boolean hasNext() {
				if (stopTime == null)
					stopTime = endtime;
				return !curTime.plus(resolution).isAfter(stopTime);
			}
	...
	public Iterator<RoutePoint> iterateBy(Duration resolution) {
		return iterateBy(starttime, endtime, resolution);
